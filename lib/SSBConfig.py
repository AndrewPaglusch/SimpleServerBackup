...
class SSBConfig:
    ...
    def _load_config(self):
        if not self.cp.read(self.configfile):
           raise Exception("Invalid or missing configuration file")
    ...
    def _sort_files(self, files):
        pre  = sorted([ file for file in files if file.startswith('pre_') ])
        post = sorted([ file for file in files if file.startswith('post_') ])
        return pre, post
